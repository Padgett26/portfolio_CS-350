...
class TemperatureMachine(StateMachine):
    ...
    ## I set this at 82 to be closer the ambient room temp
    setPoint = 82
    ...
    def updateLights(self):
        ...
        if self.current_state.id == "heat":
            # temp is at or above the setPoint
            if temp >= self.setPoint:
                redLight.on()
            # temp is below the setPoint
            else:
                redLight.pulse()
        # in cool state
        elif self.current_state.id == "cool":
            # temp is at or below the setPoint
            if temp <= self.setPoint:
                blueLight.on()
            # temp is above the setPoint
            else:
                blueLight.pulse()
        else:
            pass
